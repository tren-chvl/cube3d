...
/*   Updated: 2025/11/07 12:43:49 by rdestruh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static ssize_t	ft_dprintf_var(va_list ap, const char id, int fd)
{
	ssize_t	len;
...
static int	main_ft_dprintf(const char *s, va_list ap, int fd)
{
	size_t	i;
	size_t	var_index;
	...
	while (s[i])
	{
		...
		{
			percent_count++;
			var_index += ft_dprintf_var(ap, s[i + 1], fd);
			i += 2;
		}
	}
